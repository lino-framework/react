...
const LinoComponents = {
        TabPanel: (props) => (
            <TabView className={classNames("lino-panel")}>
                {props.elem.items.filter((e) => !e.hidden).map((panel, i) => {
                        let Child = props.linoLayout.getComponent(panel.react_name);
                        return <TabPanel header={panel.label} key={key(panel)} contentClassName={"lino-panel"}>
                            <Child {...props} elem={panel} header={false}/>
                        </TabPanel>
                    }
                )
                }
        ...
        Panel: (props) => {
            ...
            const children = props.elem.items.filter((e) => !e.hidden).map((child, i) => {
                let Child = props.linoLayout.getComponent(child.react_name);
                let style = {};
                if (child.value.flex) {
        ...
        ActionParamsPanel: (props) => {
            let Panel = props.linoLayout.getComponent("panel"),
                {action, elem, onSubmit} = props,
            ...
            return (
                <form target={is_sign_in ? "temp" : undefined}
                    ...
                    <Panel {...props} elem={elem} action_dialog={true}/>
                    <input type="submit"
...
class LinoLayout extends React.Component {
    ...
    render() {
        ...
        const Main = this.getComponent(elem.react_name);
        return <Main {...this.props} elem={elem} linoLayout={this}/>
    ...
    getComponent(name) {
        ...
        if (this.firstFocusable === undefined && Child.focus){
            this.firstFocusable = Child.focus;
        }
        return Child
    }
